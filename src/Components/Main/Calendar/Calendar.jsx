...
const Calendar = (props) => {
    ...
    useEffect(() => {
        axios
            .get('http://localhost:1337/api/events?populate=*&filters[dateStart][$gte]=2022-10-14T14:00:00.000Z&filters[dateStart][$lte]=2024-10-14T14:00:00.000Z')
            .then((response) => {
                                ...
                                week.map( (day, index) => {
                                    return(
                                        <div key={index} className={isCurrentMonthDate(day.date) ? 'day' : 'day non-current'}>
                                            ...
                                            {
                                                hasEvents(day.date) && currentEvent != null ? 
                                                <span id={currentEvent.id} className='event-calendar' onClick={handlerClickEvents}>{currentEvent.title}</span>
                                                :
                                                null
                                            }
                                        </div>
